...
func TestM163CrewAIAuditExportShape(t *testing.T) {
	...
	if !strings.Contains(joined, "llm_result") || !sawLLM {
		t.Fatalf("loopback crewai path missing llm_result events: %q", types)
	}
	if !sawEgress {
		t.Fatalf("loopback crewai path missing egress events: %q", types)
	}
...
func TestM163LoopbackModels(t *testing.T) {
...
func TestM163WorkloadEnvOmitsProviderKey(t *testing.T) {
	const planted = "M163-PLANTED-PROVIDER-KEY-NOT-DUMMY"
...
func TestM163DirectProviderHostDenied(t *testing.T) {
	forbidden := []string{
	...
	env := workerEnvOpenAI([]string{
		...
		"OPENAI_API_KEY=M163-PLANTED-PROVIDER-KEY-NOT-DUMMY",
		"OTEL_EXPORTER_OTLP_ENDPOINT=https://telemetry.crewai.com",
		"CREWAI_API_KEY=M163-PLANTED-PROVIDER-KEY-NOT-DUMMY",
	}, "127.0.0.1:1", "http://127.0.0.1:54321/v1")
	for _, item := range env {
...
func TestM163CrewAITelemetryOff(t *testing.T) {
	env := workerEnvOpenAI([]string{
		"PATH=/usr/bin",
		"CREWAI_API_KEY=planted-crewai-key",
		"OTEL_EXPORTER_OTLP_ENDPOINT=https://telemetry.crewai.com",
		"OTEL_EXPORTER_OTLP_TRACES_ENDPOINT=https://app.posthog.com",
	}, "127.0.0.1:1", "http://127.0.0.1:54321/v1")
	joined := strings.Join(env, "\n")
	if !strings.Contains(joined, "CREWAI_DISABLE_TELEMETRY=true") {
		t.Fatal("workerEnvOpenAI must set CREWAI_DISABLE_TELEMETRY=true")
	}
	if strings.Contains(joined, "telemetry.crewai.com") {
		t.Fatal("workerEnvOpenAI must not contain telemetry.crewai.com")
	}
	if !pydanticDirectHostDenied("telemetry.crewai.com") {
		t.Fatal(`pydanticDirectHostDenied("telemetry.crewai.com") = false, want true`)
	}
}
