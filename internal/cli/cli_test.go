...
func TestAgentVersion_JSONOutput(t *testing.T) {
	...
	if v.CLIVersion != "0.4.2-dev" {
		t.Errorf("cli_version: want %q, got %q", "0.4.2-dev", v.CLIVersion)
	}
...
// command output reports 0.4.2-dev when built with the default ldflags
...
func TestVersionCommand_ContainsCurrentDevVersion(t *testing.T) {
	...
	// The version must contain 0.4.2 — stale 0.1.x or 0.2.x strings are rejected.
	if !strings.Contains(stdout, "0.4.2") {
		t.Errorf("version output should contain 0.4.2 (dev or tagged), got:\n%s", stdout)
	}
