...
| **P2-14** | Docs send first-time users to `cloud login` with no claim-link guidance — "Open your claim link first" error | **OPEN — 2026-08-03 founder E2E** — bare `agentpaas cloud login` (browser mode) errors "Open your claim link first — you need to sign in via a claim link before you can approve CLI logins"; docs only showed --token-stdin and never mention the claim link (README/quickstart had no cloud section at all). Fix in flight: golden-path.md restructured to claim-link-primary + token fallback; quickstart gains a cloud section; README consistency (worker dispatched) |
