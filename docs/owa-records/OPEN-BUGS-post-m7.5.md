...
|| **P2-19** | Cloud browser auth pages (claim + Approve CLI Login) are blank white — unstyled | **FIXED eng on cloud main `51181b7` — pending production deploy**. Both **claim-link** and **Approve CLI Login** pages now have the dashboard styling; production deployment remains pending. See `BUG-P2-19-cli-login-approve-white-page.md`. |
|| **P2-20** | Dashboard refresh while logged in flashes “AGENTPAASCLOUD / Checking your claim” then needs another refresh for real UI | **PARTIAL** — loading copy fixed eng; session bootstrap/claim-check behavior still needs production verification. See `BUG-P2-20-dashboard-refresh-claim-flash.md`. |
| **P2-21** | Trial/stranger auth incomplete: need real browser login (Google and/or username+password), log off, stable account after claim | **OPEN 2026-08-05** — founder. Claim alone is not enough; after trial claim user must sign in/out with **their** identity via browser. See `BUG-P2-21-trial-browser-login-logoff.md`. |
|| **P2-22** | Trial CPU metering not proven correct in founder/golden path | **PARTIAL** — golden asserts CPU delta live-proven; dashboard presentation and longer-lived production evidence remain pending. See `BUG-P2-22-trial-cpu-metering-and-expiry.md`. |
|| **P2-23** | Trial >30 days expiry UX + hard block (push/invoke/runs) unproven | **PARTIAL** — edge cases 10–12 PASS with admin PATCH + founder messages in `quota.ts` (cloud) and CLI; full dashboard live pending CF deploy. See `BUG-P2-22-trial-cpu-metering-and-expiry.md`. |
|| **P2-28** | CPU quota exhausted: dashboard + CLI must show same “trial limits crossed → new trial or paid” message and block work | **PARTIAL** — edge cases 10–12 PASS with admin PATCH + founder messages in `quota.ts` (cloud) and CLI; full dashboard live pending CF deploy. See `BUG-P2-28-cpu-exhausted-trial-upgrade-message.md`. |
|| **P2-29** | Trial **>10 agents**: control plane hard error + CLI “convert to paid” (future feature / test) | **PARTIAL** — edge cases 10–12 PASS with admin PATCH + founder messages in `quota.ts` (cloud) and CLI; full dashboard live pending CF deploy. See `BUG-P2-29-trial-agent-limit-10-upgrade.md`. |
|| **P2-24** | Cron-triggered agents never covered in founder golden | **PARTIAL** — cron tick smoke edge added to `scripts/golden-path-edge-regressions.sh`; live schedule → fire → run → result remains dependent on the run's live prerequisites. See `BUG-P2-24-cron-agent-golden.md`. |
|| **P2-25** | Dashboard does not show per-agent **direct invoke endpoint** | **PARTIAL** — inspect/invoke URL eng on cloud main `1b155d1+`; full production dashboard verification remains pending. See `BUG-P2-25-dashboard-invoke-endpoint.md`. |
|| **P2-26** | Agent **Inspect** missing full lineage / fingerprint / security details | **PARTIAL** — inspect/invoke URL eng on cloud main `1b155d1+`; full production dashboard verification remains pending. See `BUG-P2-26-dashboard-agent-inspect-lineage.md`. |
|| **P2-27** | Dashboard has no full agent log view (ingress / egress / messages) | **OPEN** — full dashboard logs (ingress / egress / messages) are still pending; CLI `cloud logs` alone is insufficient. See `BUG-P2-27-dashboard-agent-logs.md`. |
