...
|----|--------|------------------|
| **UX-DLOG** | FIXED 2026-08-02 — root `SilenceErrors`; main prints single `error:` | was cobra+main double print |
| **UX-DDOCKER** | FIXED 2026-08-02 — live `docker context show` + `docker info` probe | was stub unknown |
| **UX-APIHOST** | FIXED messaging 2026-08-02 — DNS failures tell you to set AGENTPAAS_CLOUD_API_URL; DNS itself still open | workers.dev until zone DNS |
| **BUG-CF-BIND** | PARTIAL — CLI now prints API `error` body (UX-HTTP-BODY); ops checklist still required post-teardown | see BUG-cf-bind-not-configured-t11.md |
| **UX-SECRETS-OUT** | FIXED 2026-08-02 — list prints `Cloud secrets (N):` + indented names | push already printed `pushed:` |
| **UX-LOCKPATH / UX-DEPLOY** | FIXED 2026-08-02 — pack prints Lock path; `deploy latest` + `--lock` | golden-path.md updated |
