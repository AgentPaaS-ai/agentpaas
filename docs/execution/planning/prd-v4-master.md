...
**Release gate:** `docs/execution/golden-loop-test.md` (pinned to v0.2.3) — every release must pass the golden loop before shipping
...
### v0.1.x continuation (Blocks 25–27 — secure portability and golden enterprise proof)

- **Block 25:** execute security dependency closure first (patched Go
  toolchain, patched Docker Engine readiness enforcement, and supported Moby
  client/API modules), then build Hermes sharing UX, run the two-person/manual
  and seven-claim adversary gates, truth-sync documentation, and only then
  promote v0.2.0/Homebrew. No release artifact is published before those
  ordered gates pass.
- **Block 26:** prove operator portability with a Codex skill over the stable
  JSON CLI, then extract shared Hermes/Codex safety rules and conformance
  tests, then produce the Agent Approval Pack. Those three chunks are required
  before B27. A generic MCP operator interface is a later evidence-based
  decision gate and does not block B27. Hosted Grok, registry/distribution,
  revocation, keyless signing, overlays, and update awareness remain
  demand-gated backlog rather than hidden B26 completion criteria.
- **Block 27:** close the capabilities required for the two-person golden
  enterprise workflow in dependency order: first prove the gateway/Broker
  header-mutation topology; then build signed keyless requirements,
  receiver-local profiles/OAuth, request-time injection and removal of legacy
  harness credential preload; then add path/resource policy, exact-action
  approval, receipts, named LLM channels, portable MCP, data-flow evidence,
  provider fixtures, Approval Pack extensions, and finally the golden E2E.
  Every listed capability is P0 for the full enterprise claim unless the demo
  and public claim are explicitly narrowed under B27's kill/de-scope rules.

The authoritative task contracts are `docs/execution/blocks/b25-summary.md`,
`docs/execution/blocks/b26-summary.md`, and
`docs/execution/blocks/b27-summary.md`.
...
### 2.5.3 Keyless agent portability and request-time injection (B27 normative)
...
forbidden for the B27 enterprise path.
...
a hosted AgentPaaS OAuth broker is not a B27 requirement. Refresh is
...
for workloads that must read a secret. They are excluded from the B27 golden
enterprise demo and cannot satisfy the keyless request-time-injection claim.
The complete implementation and adversary gates are normative in
`docs/execution/blocks/b27-summary.md`.
