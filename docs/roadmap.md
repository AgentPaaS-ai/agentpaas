...
## Release Gate — Golden Loop Test

Every public release must pass the [Golden Loop Test](execution/golden-loop-test.md)
before shipping. Pinned to v0.2.3. The golden loop exercises the full lifecycle:
clean install → build agent → modify → provenance → export → teardown →
receive-as-friend → inspect → install → run. Update the test file for each
new release.
