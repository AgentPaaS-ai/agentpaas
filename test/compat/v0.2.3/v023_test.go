...
// TestCharacterization_DaemonInvokeTimeout is a source-location characterization
// proving the daemon uses a 2-minute invoke timeout.
// Source: internal/daemon/control_handlers.go:794
func TestCharacterization_DaemonInvokeTimeout(t *testing.T) {
	t.Log("BOUNDARY: Daemon invoke timeout = 2 minutes")
	t.Log("SOURCE: internal/daemon/control_handlers.go:794")
	t.Log("  timeoutCtx, timeoutCancel := context.WithTimeout(invokeCtx, 2*time.Minute)")
	t.Log("EFFECT: A run whose inner blocking request exceeds ~2 minutes fails")
	t.Log("  at the daemon level before harness budget expires.")
	t.Log("CONFLICT: Harness default wall clock budget = 120 seconds (internal/harness/budget.go:17)")
	t.Log("CONFLICT: Model client HTTP timeout = 120 seconds (internal/harness/rpc_server.go:402)")
	t.Log("CONFLICT: Daemon timeout = 2 minutes = 120 seconds")
	t.Log("CONFLICT: Three independent timers at ~120s each — first one to fire wins")
	t.Log("CONFLICT: No single authoritative source of timeout; layering is accidental.")
}

// TestCharacterization_HarnessBudgetDefault proves the default harness budget values.
// Source: internal/harness/budget.go:17-19
func TestCharacterization_HarnessBudgetDefault(t *testing.T) {
	t.Log("BOUNDARY: Harness default wall clock budget = 120 seconds")
	t.Log("SOURCE: internal/harness/budget.go:17")
	t.Log("  const defaultWallClockBudget = 120 * time.Second")
	t.Log("BOUNDARY: Harness default max iterations = 10000")
	t.Log("SOURCE: internal/harness/budget.go:18")
	t.Log("  const defaultMaxIterations = 10000")
	t.Log("BOUNDARY: Harness default max tokens = 100000")
	t.Log("SOURCE: internal/harness/budget.go:19")
	t.Log("  const defaultMaxTokens = 100000")
	t.Log("CONFLICT: 120s daemon timeout + 120s harness budget + 120s model client timeout")
	t.Log("  means no single layer owns the timeout. If a model call takes 61s,")
	t.Log("  the Python worker wall timer (120s) won't fire, but the daemon context")
	t.Log("  (2m from invoke start) may fire before a second long call completes.")
}

// TestCharacterization_ModelClientTimeout proves the 120-second HTTP client timeout.
// Source: internal/harness/rpc_server.go:402
func TestCharacterization_ModelClientTimeout(t *testing.T) {
	t.Log("BOUNDARY: Model client HTTP timeout = 120 seconds")
	t.Log("SOURCE: internal/harness/rpc_server.go:402")
	t.Log("  client := &http.Client{Timeout: 120 * time.Second, ...}")
	t.Log("EFFECT: An LLM HTTP request that takes >120s gets a timeout at the")
	t.Log("  HTTP client layer BEFORE the harness wall clock budget (also 120s)")
	t.Log("  or daemon invoke timeout (2min) fire, since the timeout is per-call.")
	t.Log("CONFLICT: HTTP timeout (120s) + harness wall clock (120s) + daemon (120s)")
	t.Log("  are all approximately equal. Ordering is accidental: whichever fires")
	t.Log("  first wins.")
}

// TestCharacterization_RLimits proves the CPU rlimit and zero child-process rlimit.
// Source: internal/harness/python_worker.go:460-462
func TestCharacterization_RLimits(t *testing.T) {
	t.Log("BOUNDARY: RLIMIT_CPU = 30 seconds")
	t.Log("SOURCE: internal/harness/python_worker.go:460")
	t.Log("  ('RLIMIT_CPU', 30)")
	t.Log("BOUNDARY: RLIMIT_NPROC = 0 (no child processes)")
	t.Log("SOURCE: internal/harness/python_worker.go:462")
	t.Log("  ('RLIMIT_NPROC', 0)")
	t.Log("BOUNDARY: RLIMIT_FSIZE = 64 MB")
	t.Log("SOURCE: internal/harness/python_worker.go:461")
	t.Log("  ('RLIMIT_FSIZE', 64 * 1024 * 1024)")
	t.Log("NOTE: These are applied inside the Python worker process via")
	t.Log("  resource.setrlimit(). Best-effort, platform-dependent.")
	t.Log("EFFECT: CPU-bound agent code exceeding ~30s CPU time gets SIGXCPU.")
	t.Log("  The agent cannot spawn subprocesses (NPROC=0).")
}
